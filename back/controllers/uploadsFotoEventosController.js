...
const cargarArchivo = async(req, res = response) => {
    ...
    try {
        ...
        const nombre = await subirArchivo( req.files, undefined, process.env.CARPETAFOTOSEVENTOS );
        res.json({ url:nombre });
...
const borrarImagen = async(req, res = response ) => {
    ...
    const pathImagen = path.join( __dirname, '../uploads', process.env.CARPETAFOTOSEVENTOS, idborrado );
...
const actualizarImagen = async(req, res = response ) => {
        ...
        const pathImagen = path.join( __dirname, '../uploads', process.env.CARPETAFOTOSEVENTOS,req.params.id);
        if ( fs.existsSync( pathImagen ) ) {
            fs.unlinkSync( pathImagen );
        }
