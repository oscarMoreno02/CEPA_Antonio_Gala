...
const cargarArchivo = async(req, res = response) => {
    ...
    try {
        ...
        const nombre = await subirArchivo( req.files, undefined, process.env.CARPETAFOTOSNOTICIAS );
        res.json({ url:nombre });
...
const borrarImagen = async(req, res = response ) => {
    ...
    const pathImagen = path.join( __dirname, '../uploads', process.env.CARPETAFOTOSNOTICIAS, idborrado );
...
const actualizarImagen = async(req, res = response ) => {
        ...
        const pathImagen = path.join( __dirname, '../uploads', process.env.CARPETAFOTOSNOTICIAS,req.params.id);
        if ( fs.existsSync( pathImagen ) ) {
            fs.unlinkSync( pathImagen );
        }
